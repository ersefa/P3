...
public class DropCommand extends Command {
	...
	public boolean execute() {
		...
		// False = Si no hay ningún Item en el inventario
		...
		// False = Si el elemento a dropear no lo tengo en el inventario
		if (itemsFoundedinInventory.isEmpty()) {
			result = msgDropCommandItemNameNotInInventory + LINE_SEPARATOR;
			return false;
		}

		// False = Tengo mas de un elemento en el inventario y no permito
		// desambiguar
		...
		// Como al menos tengo un elemento con ese nombre cojo el primero
		firstItem = (Item) itemsFoundedinInventory.toArray()[0];

		itemsFoundedinLocation = game.getItemsFromLocation(itemName);

		// False = El elemento que intento dropear ya existe en la localizacion
